-- Seed data để test dashboard
-- Chạy script này để thêm dữ liệu mẫu cho dashboard
...
-- 5. Thêm Appointments (30 ngày qua)
...
-- 6. Thêm Invoices (đơn giản - không sử dụng appointment_id)
...
BEGIN
    -- Sử dụng lại các biến đã khai báo trước đó
    SET @patient1_id = (SELECT patient_id FROM Patients WHERE user_id = (SELECT user_id FROM Users WHERE username = 'patient1'));
    SET @patient2_id = (SELECT patient_id FROM Patients WHERE user_id = (SELECT user_id FROM Users WHERE username = 'patient2'));
    SET @patient3_id = (SELECT patient_id FROM Patients WHERE user_id = (SELECT user_id FROM Users WHERE username = 'patient3'));
    SET @patient4_id = (SELECT patient_id FROM Patients WHERE user_id = (SELECT user_id FROM Users WHERE username = 'patient4'));
    SET @patient5_id = (SELECT patient_id FROM Patients WHERE user_id = (SELECT user_id FROM Users WHERE username = 'patient5'));
...
-- 7. Thêm Payments (sử dụng invoice_id thực tế)
