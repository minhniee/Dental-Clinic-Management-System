...
public class DashboardServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            int totalAppointments = getTotalAppointments(); // có thể đổi sang theo period nếu muốn
            ...
            double totalRevenue = getTotalRevenue(); // có thể đổi sang theo period nếu muốn
    ...
    private int getTotalAppointments() throws SQLException {
        String sql = "SELECT COUNT(*) FROM Appointments WHERE created_at >= DATEADD(day, -30, GETDATE())";
        try (Connection conn = new DBContext().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            int result = rs.next() ? rs.getInt(1) : 0;
    ...
    private double getTotalRevenue() throws SQLException {
        String sql = "SELECT ISNULL(SUM(total_amount), 0) FROM Invoices WHERE status = 'PAID' AND created_at >= DATEADD(day, -30, GETDATE())";
        try (Connection conn = new DBContext().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
    ...
    private List<Map<String, Object>> getAppointmentTrends(int days) throws SQLException {
        String sql = "SELECT CAST(created_at AS DATE) as date, COUNT(*) as count " +
                "FROM Appointments " +
